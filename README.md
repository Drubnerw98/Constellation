...
- **In-canvas hover labels.** Hovering a node surfaces its title right next to the star and simultaneously labels every connected neighbor, so a node's web is scannable in place. Cluster labels run a collision-resolution pass so anchors never stack on top of each other.
